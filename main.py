...
uploaded_query_file = st.file_uploader(
    "📂 (Optional) Upload a file with extra details",
    type=["txt", "pdf", "docx", "png", "jpg", "jpeg"]
)

file_text = ""
if uploaded_query_file:
    file_text = extract_text_from_file(uploaded_query_file)

# Merge inputs (if both given)
user_query = ""
if text_query and file_text:
    user_query = text_query.strip() + "\n\nAdditional details from file:\n" + file_text.strip()
elif text_query:
    user_query = text_query.strip()
elif file_text:
    user_query = file_text.strip()
